...
export default function Publisher() {
  const { functions, publisher, loading } = useContext(FunctionContext);
  const { fetchData, fetchPublisher } = functions;

  const [status, setStatus] = React.useState(false);

  useEffect(() => {
    if (!status && publisher.length === 0) {
      fetchPublisher();
      setStatus(true);
    }
  }, [status]);
    ...
    <Container>
      <div>
        ...
        <div className="header">
          <h1 className="text-slate-700 text-3xl font-sans my-3">Publisher</h1>
          <div></div>
        </div>
        {publisher ? (
